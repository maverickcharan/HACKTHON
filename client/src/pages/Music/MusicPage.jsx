  ...
  useEffect(() => {
    ...
    if (userPreferences.selectedLanguages && userPreferences.selectedLanguages.length > 0) {
      result = result.filter((music) => {
        // If music has no language property, show it
        if (!music.language) return true;
        // Check if music language matches any selected language
    ...
    if (userPreferences.selectedMood) {
      ...
      if (recommendedCategories.length > 0 && selectedCategories.length === 0) {
        ...
        result = result.filter((m) => recommendedCategories.includes(m.category));
      }
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0B1020] to-[#0F172A] text-white">
      ...
      <div className="p-6 border-b border-gray-700">
        <h1 className="text-2xl md:text-3xl font-bold">Music Recommendations</h1>
        <p className="text-white/60 mt-2">
          Based on your preferences • {filteredMusic.length} tracks/artists found
        </p>
      </div>
